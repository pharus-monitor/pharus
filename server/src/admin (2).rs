...
pub fn router(state: SharedState) -> Router<SharedState> {
    let protected = Router::new()
        ...
        .route(
            "/api/admin/agents/:id/ssh",
            get(ssh_info).put(save_ssh_creds).delete(clear_ssh_creds),
        )
...
/// Whether SSH credentials exist for a host (never returns the secret).
async fn ssh_info(State(state): State<SharedState>, Path(id): Path<i64>) -> Response {
    let configured = crate::ssh_term::load_creds(&state, id)
        .ok()
        .flatten()
        .map(|c| !c.user.is_empty())
        .unwrap_or(false);
    let host = crate::ssh_term::ssh_target(
        &state,
        id,
        &crate::ssh_term::SshCreds::default(),
    );
    Json(serde_json::json!({
...
async fn save_ssh_creds(
    State(state): State<SharedState>,
    Path(id): Path<i64>,
    Json(body): Json<SshCredsBody>,
) -> Response {
    if body.user.trim().is_empty() || body.password.is_empty() {
        return bad("user and password are required");
    }
    if let Some(h) = &body.host {
        let h = h.trim();
        if !h.is_empty()
            && !h.split(':').next().map(|s| !s.is_empty() && s.chars().all(|c| c.is_ascii_alphanumeric() || c == '.' || c == '-')).unwrap_or(false)
        {
            return bad("host must be a hostname or IP (host[:port] not supported, use the port field)");
        }
    }
    let creds = crate::ssh_term::SshCreds {
        host: body.host.as_deref().map(str::trim).filter(|s| !s.is_empty()).map(String::from),
        port: body.port.unwrap_or(22),
        user: body.user.trim().to_string(),
        password: body.password,
    };
    match crate::ssh_term::save_creds(&state, id, &creds) {
        Ok(()) => StatusCode::OK.into_response(),
        Err(e) => bad(&e.to_string()),
    }
}

async fn clear_ssh_creds(State(state): State<SharedState>, Path(id): Path<i64>) -> Response {
    match crate::ssh_term::clear_creds(&state, id) {
        Ok(()) => StatusCode::OK.into_response(),
        Err(e) => bad(&e.to_string()),
    }
}
