...
pub enum AgentMsg {
    ...
    /// Docker containers on the agent host. `available` is false when the
    /// agent could not reach a Docker daemon (panel hidden); true with an
    /// empty list means Docker is running with no containers.
    Containers {
...
pub struct AgentSnapshot {
    ...
    /// Docker containers reported by the agent. None = no Docker daemon on
    /// the host (panel hidden); Some(empty) = Docker running, no containers.
    #[serde(default)]
    pub containers: Option<Vec<ContainerInfo>>,
    /// True once the agent reported Docker availability at least once
    /// (cached across brief daemon hiccups).
    #[serde(default)]
